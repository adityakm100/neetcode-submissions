class MinStack:
    ...
    def pop(self) -> None:
        ...
        self.minStack.pop() #pop from minStack, since we ALWAYS add a value to minStack, even if we remove the current minimum, its also stored safely as the next element behind it

    def top(self) -> int:
        return self.stack[-1] #return last element of the stack

    def getMin(self) -> int:
        return self.minStack[-1] #return last element of minStack which is guaranteed to be current min

    
    #EXAMPLE OF THE TWO STACK APPROACH
